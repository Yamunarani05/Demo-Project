...
import { Search, Eye, ArrowLeft, Briefcase, CalendarDays, Upload, RotateCcw, CheckCircle, MapPin, User, FileText, Shirt, Palette, UserPlus, Image, Download, Plane , ExternalLink, Link2, ImageIcon} from 'lucide-react'
...
export default function DroneAssignedClient() {
    ...
    const [cr3Mode, ] = useState<'with_cr3' | 'without_cr3'>('without_cr3')
    ...
    const filtered = leads.filter(l => {
        ...
        const isDroneTask = l.task_name?.toLowerCase().includes('drone');
        return matchesSearch && isDroneTask;
    })

    const fetchClientDetails = async (leadId: number | string, flowStage?: string, taskName?: string, _taskKey?: string) => {
        ...
        try {
            ...
            if (statusRes.data?.success && statusRes.data.data && Array.isArray(statusRes.data.data.accepted_assignments)) {
                ...
                if (user) {
                    ...
                    let currentTaskKey = _taskKey || selectedLead?.task_key || taskName || ''
                                        ...
                                        <div key={lead.lead_employee_id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-3">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-xs font-medium text-teal-600">{lead.lead_code || `LD-${lead.lead_id}`}</p>
                                                </div>
                                                {lead.accepted ? (
    ...
    return (
        <div className="max-w-[1400px] mx-auto p-4 md:p-8 animate-in fade-in zoom-in-95 duration-300">
            <Breadcrumb items={[{ label: 'Assigned Clients', link: '/multi-role/drone/assigned' }, { label: selectedLead.name }]} homeLink="/multi-role/dashboard" />
