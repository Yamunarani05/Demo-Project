...
for (let file of files) {
    ...
    // )
    // }
    content = content.replace(/<\/table>[\s\S]*?<\/div>\r?\n\s*\)\r?\n\s*\}/, `</table>
                    </div>
                )}
            </div>
        )
    ...
    // 2. Fix the broken Shoot Locations Section / end of renderClientDetails
    // It currently has something like:
    // {/* Location Details */} ...
    // </div>
    // )
    ...
    // )
    // }
    const locationDetailsRegex = /(\{\/\*\s*Location Details\s*\*\/\}(?:[\s\S](?!\{\/\*))*?)<\/div>\s*\)\s*\}\s*<\/div>\s*\)\s*\}/;
    const replacement = `$1
                    </div>
                )}
    ...
    content = content.replace(locationDetailsRegex, replacement);

    // 3. Fix the Upload Tab end
    // It has:
    ...
    // )
    // }
    const uploadTabRegex = /(Send to Data Manager\s*<\/button>\s*<\/div>)\s*\)\s*\}/;
    content = content.replace(uploadTabRegex, `$1
                        )}
                    </div>
                )}`);
