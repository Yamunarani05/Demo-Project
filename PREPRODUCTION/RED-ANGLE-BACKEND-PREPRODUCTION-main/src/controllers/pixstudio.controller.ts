...
import { submitPixstudioDataService, getPixstudioStatsService } from "../services/pixstudio.service";
import { pool } from "../config/db";
import { createNotificationService } from "../services/notification.service";

export const submitPixstudioDataController = async (req: Request, res: Response) => {
    try {
        const body = req.body;
        if (!body.external_lead_id || !body.event_name) {
            return res.status(400).json({ success: false, message: "Missing required fields" });
        }
        const data = await submitPixstudioDataService(body);
        
        try {
            const { updateCurrentStageService } = require('../services/stageTracking.service');
            await updateCurrentStageService(body.external_lead_id, 'pixstudio_processing');
            await updateCurrentStageService(body.external_lead_id, 'assigned_to_crm');
        } catch (e) {
            console.error('Failed to trigger Pixstudio stage tracking:', e);
        }
        ...
        console.error("SUBMIT PIXSTUDIO ERROR:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};

export const getPixstudioStatsController = async (req: Request, res: Response) => {
    try {
        const stats = await getPixstudioStatsService();
        res.status(200).json({ success: true, data: stats });
    } catch (error: any) {
        console.error("GET PIXSTUDIO STATS ERROR:", error);
        res.status(500).json({ success: false, message: error.message });
    }
};
