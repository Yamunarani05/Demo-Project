...
for (let file of files) {
    ...
    // 1. Remove the badly placed Shoot Locations Section
    const badPlacementRegex = /\{\/\* Shoot Locations Section \*\/\}[\s\S]*?<\/div>[\r\n\s]*\)[\r\n\s]*\}/;
    if (content.match(badPlacementRegex)) {
        content = content.replace(badPlacementRegex, '</div>\n        )\n    }');
    }

    // 2. Insert it at the ACTUAL end of renderClientDetails()
    // It looks like:
    /*
        {/* Location Details * /}
    ...
    */
    // We can just find the end of `renderClientDetails` by finding `{/* Location Details */}` and the following `</div>\n        )\n    }`
    
    // Split by `{/* Location Details */}`
    const parts = content.split('{/* Location Details */}');
    if (parts.length > 1) {
        let afterLocationDetails = parts[1];
        afterLocationDetails = afterLocationDetails.replace(/<\/div>[\r\n\s]*\)[\r\n\s]*\}/, shootLocationsStr);
        content = parts[0] + '{/* Location Details */}' + afterLocationDetails;
    }
