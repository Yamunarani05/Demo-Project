...
import { getWorkflowStatusMeta, isPixofficeReady, isPixofficeSubmitted } from '../utils/workflowStatus'
                ...
                if (res.data.success && res.data.data) {
                    const mappedData = res.data.data
                        ...
                        .filter((item: any) => {
                            ...
                            return id.startsWith('RAS');
                        })
                        .filter((item: any) => isPixofficeReady(item.status) || isPixofficeSubmitted(item.status))
                        .map((item: any) => ({
    ...
    const filteredData = useMemo(() => {
        return incomingData.filter((item) => {
            ...
            const matchesStatus = statusFilter === 'All' || 
                                  ...
                                  (statusFilter === 'QC_Pending_Pixoffice' && item.status === 'QC_Pending_Pixoffice')
    ...
    if (submitted) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-sm mb-6" style={{ color: '#6B7280' }}>Pixoffice details saved. Status: <b style={{ color: '#059669' }}>Submitted to CRM</b></p>
                <div className="flex gap-3">
                    <span className="px-4 py-2 rounded-full text-xs font-bold" style={{ background: '#dcfce7', color: '#059669' }}>Pixoffice Complete</span>
            ...
            <div>
                ...
                <div className="flex gap-4 mb-6 relative">
                    ...
                    <div className="relative">
                        ...
                        >
                            <Filter size={16} />
                            {statusFilter === 'All' ? 'Filter' : statusFilter === 'QC_Pending_Pixoffice' ? 'Submitted' : statusFilter.replace('_', ' ')}
                        ...
                        {isFilterOpen && (
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 z-20 py-1 overflow-hidden">
                                    {['All', 'Pending', 'Submitted', 'QC_Completed'].map((status) => (
                                        <button
                                            key={status}
                                            onClick={() => {
                                                setStatusFilter(status === 'Submitted' ? 'QC_Pending_Pixoffice' : status)
                                                setIsFilterOpen(false)
                                            }}
                                            className={`w-full text-left px-4 py-2 text-sm hover:bg-purple-50 transition-colors ${(statusFilter === status || (status === 'Submitted' && statusFilter === 'QC_Pending_Pixoffice')) ? 'text-purple-700 font-bold bg-purple-50' : 'text-gray-700 font-medium'}`}
                                        >
                                            {status.replace('_', ' ')}
                                        </button>
                                    ))}
                                ...
                                filteredData.map((row) => (
                                    <tr key={row.id} className="border-b border-gray-50 last:border-none hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-5 py-4">
                                            {(() => {
                                                const meta = getWorkflowStatusMeta(row.status)
                                                return (
                                                    <span className={`px-3 py-1 text-[10px] font-bold rounded-lg ${meta.badgeClass}`}>
                                                        {row.status === 'Verified' ? 'Ready for Pixoffice' : meta.label}
                                                    </span>
                                                )
                                            })()}
                                        </td>
                                        <td className="px-5 py-4">
                                            {row.status === 'QC_Pending_Pixoffice' ? (
                                                <span className="px-4 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold">
                                                    ✓ Submitted
                                                </span>
