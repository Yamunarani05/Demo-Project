...
const typeIcons: Record<string, { emoji: string; gradient: string }> = {
  ...
  'Traditional Video Editing': { emoji: '🎞️', gradient: 'linear-gradient(135deg, #4F46E5, #818CF8)' },
  'Retouch Editing': { emoji: '📷', gradient: 'linear-gradient(135deg, #0891B2, #22D3EE)' },
  'Album Design': { emoji: '🖼️', gradient: 'linear-gradient(135deg, #7C3AED, #A78BFA)' },
  'Candid Video Editing': { emoji: '🎥', gradient: 'linear-gradient(135deg, #DB2777, #F472B6)' },
}
...
export default function EditApprovalDetail() {
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <button
          onClick={() => navigate('/admin/edit-approval')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {cardTypes.map(type => {
          ...
          return (
            <div
              key={type}
              onClick={() => assignment && navigate(`/admin/edit-approval/${projectId}/${encodeURIComponent(type)}`)}
