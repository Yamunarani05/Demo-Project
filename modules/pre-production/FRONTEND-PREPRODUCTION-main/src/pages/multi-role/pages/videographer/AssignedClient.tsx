...
import { Upload, RotateCcw, Search, Eye, Video, ArrowLeft, CheckCircle, User, CalendarDays, MapPin, Palette, Shirt, FileText, UserPlus, Briefcase, Image, Download, X , ExternalLink, Link2, ImageIcon} from 'lucide-react'
...
interface EventDetails {
    ...
    video_included_file_format?: string
}
...
export default function VideographerAssignedClient() {
    ...
    const [, setNumVideos] = useState('')
    const [, setUploadNotes] = useState('')
    ...
    const [cr3Mode, _] = useState<'with_cr3' | 'without_cr3'>('without_cr3')
    const [firstClip, setFirstClip] = useState<string | File>('')
    const [lastClip, setLastClip] = useState<string | File>('')
    const [previewImage, setPreviewImage] = useState<string | null>(null)
    const [mp4Mode, setMp4Mode] = useState<'with_mp4' | 'other'>('with_mp4')
    const [mp4OtherReason, setMp4OtherReason] = useState('')
    ...
    const filtered = leads.filter(l => {
        ...
        const isVideoTask = l.task_name?.toLowerCase().includes('video');
        return matchesSearch && isVideoTask;
    })
    ...
    const fetchClientDetails = async (leadId: number | string, flowStage?: string, taskName?: string, taskKey?: string) => {
        ...
        try {
            ...
            if (eventData && eventData.client_name) {
                ...
                const isSecondary = currentTaskKey === 'event-secondary-videography' || currentTaskKey === 'secondary-videography'
                
                const existingLink = isSecondary ? (eventData.secondary_video_drive_link || '') : (eventData.video_drive_link || '')
                const existingNotes = isSecondary ? (eventData.secondary_video_upload_notes || '') : (eventData.video_upload_notes || '')
                ...
                if (linkBelongsToAssignment || hardDiskBelongsToAssignment) {
                    ...
                    const savedFormat = eventData.video_included_file_format || 'with_mp4'
                    if (savedFormat === 'with_mp4') {
                        setMp4Mode('with_mp4')
                        setMp4OtherReason('')
                    } else {
                        setMp4Mode('other')
                        setMp4OtherReason(savedFormat)
                    }
    ...
    const handleUploadSubmit = async () => {
        ...
        const shootDetails = {
            ...
            included_file_format: mp4Mode === 'with_mp4' ? 'with_mp4' : mp4OtherReason
        }
        ...
        formData.append('video_included_file_format', mp4Mode === 'with_mp4' ? 'with_mp4' : mp4OtherReason)
        if (firstClip && typeof firstClip === 'string') formData.append('first_clip_base64', firstClip)
        if (lastClip && typeof lastClip === 'string') formData.append('last_clip_base64', lastClip)
                                        ...
                                        <div key={lead.lead_employee_id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-3">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-xs font-medium text-green-600">{lead.lead_code || `LD-${lead.lead_id}`}</p>
                                                </div>
                                                {lead.accepted ? (
                        ...
                        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                            ...
                            {eventDetails?.invitation_upload ? (
                                <div className="space-y-4">
                                    ...
                                    <a href={eventDetails.invitation_upload} download="invitation_image" className="inline-flex items-center justify-center w-full gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-bold hover:bg-indigo-100 transition">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {creativeDetails.color_preferences.map((c, i) => (
                                                    <span key={i} className="px-3 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">{c}</span>
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {creativeDetails.reference_images.map((img, i) => (
                                                    <img key={i} src={`${API_URL?.replace('/api', '')}/uploads/${img}`} alt={`ref-${i}`} className="w-20 h-20 object-cover rounded-lg border border-gray-200" />
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {creativeDetails.base64_images.map((img, i) => (
                                                    <img key={`b64-${i}`} src={img} alt={`b64-ref-${i}`} className="w-20 h-20 object-cover rounded-lg border border-gray-200" />
                                                ))}
                                            </div>
                                        </div>
                                    )}
    ...
    return (
        <div className="max-w-[1400px] mx-auto p-4 md:p-8 animate-in fade-in zoom-in-95 duration-300">
            <Breadcrumb items={[{ label: 'Assigned Clients', link: '/multi-role/videographer/assigned' }, { label: selectedLead.name }]} homeLink="/multi-role/dashboard" />
                ...
                {activeTab === 'upload' && (
                    uploadLocked ? (
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
                            ...
                            <button onClick={() => setUploadSuccess(false)} className="mt-4 px-5 py-2 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors inline-flex items-center gap-2">
                                Edit Upload
                            </button>
                        </div>
                    ) : (
                        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 mb-1">MP4 Included?</label>
                                    <div className="flex gap-2 mt-1">
                                        <button type="button" onClick={() => setMp4Mode('with_mp4')}
                                            className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border transition-colors ${mp4Mode === 'with_mp4' ? 'border-green-300 bg-green-50 text-green-700' : 'border-gray-200 bg-white text-gray-500'}`}>
                                            With MP4
                                        </button>
                                        <button type="button" onClick={() => setMp4Mode('other')}
                                            className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border transition-colors ${mp4Mode === 'other' ? 'border-green-300 bg-green-50 text-green-700' : 'border-gray-200 bg-white text-gray-500'}`}>
                                            Other
                                        </button>
                                    </div>
                                    {mp4Mode === 'other' && (
                                        <div className="mt-3">
                                            ...
                                            <input type="text" value={mp4OtherReason} onChange={e => setMp4OtherReason(e.target.value)}
                                                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-100"
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-5">
                                <div><label className="block text-xs font-semibold text-gray-600 mb-1">First Clip / File Name</label>{typeof firstClip === 'string' && firstClip !== '' ? (<div className="flex items-center gap-2">{firstClip.match(/\.(jpg|jpeg|png|gif|webp|bmp)$/i) || firstClip.startsWith('data:image') ? (<button type="button" onClick={() => setPreviewImage(firstClip.startsWith('data:image') ? firstClip : `${API_URL?.replace('/api', '')}${firstClip}`)} className="w-12 h-12 rounded-lg border border-green-200 overflow-hidden hover:opacity-80 transition-opacity"><img src={firstClip.startsWith('data:image') ? firstClip : `${API_URL?.replace('/api', '')}${firstClip}`} alt="First clip" className="w-full h-full object-cover" /></button>) : (<span className="text-sm text-green-600 bg-green-50 px-3 py-1 rounded-lg">Uploaded</span>)}<button onClick={() => setFirstClip('')} className="text-xs text-red-500 hover:text-red-700 underline">Replace</button></div>) : (<input type="file" onChange={e => e.target.files?.[0] && handleFileToBase64(e.target.files[0], setFirstClip as any)} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-100 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100" />)}</div>
                                <div><label className="block text-xs font-semibold text-gray-600 mb-1">Last Clip / File Name</label>{typeof lastClip === 'string' && lastClip !== '' ? (<div className="flex items-center gap-2">{lastClip.match(/\.(jpg|jpeg|png|gif|webp|bmp)$/i) || lastClip.startsWith('data:image') ? (<button type="button" onClick={() => setPreviewImage(lastClip.startsWith('data:image') ? lastClip : `${API_URL?.replace('/api', '')}${lastClip}`)} className="w-12 h-12 rounded-lg border border-green-200 overflow-hidden hover:opacity-80 transition-opacity"><img src={lastClip.startsWith('data:image') ? lastClip : `${API_URL?.replace('/api', '')}${lastClip}`} alt="Last clip" className="w-full h-full object-cover" /></button>) : (<span className="text-sm text-green-600 bg-green-50 px-3 py-1 rounded-lg">Uploaded</span>)}<button onClick={() => setLastClip('')} className="text-xs text-red-500 hover:text-red-700 underline">Replace</button></div>) : (<input type="file" onChange={e => e.target.files?.[0] && handleFileToBase64(e.target.files[0], setLastClip as any)} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-100 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100" />)}</div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setPreviewImage(null)}>
                    <div className="relative max-w-3xl max-h-[90vh] mx-4" onClick={e => e.stopPropagation()}>
                        <button
                            ...
                            className="absolute -top-3 -right-3 z-10 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-gray-100 transition-colors border border-gray-200"
                            title="Close preview"
                        >
