            ...
            .then(result => {
                if (result.success) {
                    const filtered = (result.data || []).filter((w: WorkItem) => {
                        ...
                        return key === 'drone-coverage' || key === 'secondary-drone-coverage' || key === 'event-drone-coverage' || key === 'event-secondary-drone-coverage' || key.startsWith('additional-staff');
                    });
        ...
        <div className="max-w-[1400px] mx-auto p-4 md:p-8 animate-in fade-in zoom-in-95 duration-300">
            ...
            {loading ? (
                ...
                <div className="flex flex-col gap-8">
                    {(() => {
                        const outdoorWorks = works.filter(w => {
                            const key = w.task_key || '';
                            return key === 'drone-coverage' || key === 'secondary-drone-coverage' || key.startsWith('additional-staff');
                        });
                        const eventWorks = works.filter(w => {
                            const key = w.task_key || '';
                            return key === 'event-drone-coverage' || key === 'event-secondary-drone-coverage';
                        });
                                                ...
                                                <tbody className="divide-y divide-gray-50">
                                                    {items.map(w => (
                                                        <tr key={w.lead_employee_id} className="hover:bg-gray-50/50">
                                                            ...
                                                            <td className="px-4 py-3 text-gray-600 flex items-center gap-1"><Plane size={13} className="text-teal-500" /> {formatUIText(w.task_name || w.name)}</td>
                                                ...
                                                <div key={w.lead_employee_id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-3">
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            ...
                                                            <p className="text-xs font-medium text-teal-600">{w.lead_code || `LD-${w.lead_id}`}</p>
                                                        </div>
                                                    </div>
                                                    <div className="bg-gray-50 rounded-xl p-3 flex flex-col gap-1 text-xs">
                                                        <div className="flex justify-between">
                                                            <span className="text-gray-500">Task:</span>
                                                            <span className="font-medium text-gray-700 flex items-center gap-1"><Plane size={11} className="text-teal-500" /> {formatUIText(w.task_name || w.name)}</span>
                                                        </div>
                                                        <div className="flex justify-between">
                                                    ...
                                                    <div className="flex justify-end pt-2 border-t border-gray-100">
                                                        {w.upload_link ? (
                                                            <button
                                                                type="button"
                                                                onClick={() => window.open(w.upload_link, '_blank', 'noopener,noreferrer')}
                                                                className="flex-1 justify-center inline-flex items-center gap-1.5 rounded-xl bg-teal-50 px-3 py-2 text-xs font-bold text-teal-700 transition hover:bg-teal-100"
                                                            >
