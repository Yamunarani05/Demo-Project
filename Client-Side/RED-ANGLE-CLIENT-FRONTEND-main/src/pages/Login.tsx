...
export default function Login() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const msg = axiosError.response?.data?.message || 'Unable to connect to server. Please try again.'
            setError(msg)
        } finally {
            setLoading(false)
        }
    ...
    const handleSendOtp = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(`${API_URL}/client-auth/forgot-password`, {
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(`${API_URL}/client-auth/verify-otp`, {
                email: forgotEmail.toLowerCase().trim(),
                otp: otpCode,
            })
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(`${API_URL}/client-auth/reset-password`, {
                email: forgotEmail.toLowerCase().trim(),
                newPassword,
            })
                        ...
                        {showForgot ? (
                            ...
                            <>
                                ...
                                <form onSubmit={handleLogin} className="space-y-5">
                                    ...
                                    {/* Email or ID */}
                                    <div className="space-y-2">
                                        <label className="block text-xs font-semibold text-slate-700">Email or Client ID</label>
                                        <input
                                            type="text"
                                            required
                                            value={email}
                                            onChange={e => { setEmail(e.target.value); setError('') }}
                                            placeholder="name@example.com or RAS-100"
                                            className="w-full px-4 py-3.5 bg-[#f4f7fb] border-2 border-transparent focus:border-purple-500 focus:bg-white rounded-xl text-sm text-slate-900 transition-all outline-none placeholder:text-slate-400"
                                        />
                                    </div>
