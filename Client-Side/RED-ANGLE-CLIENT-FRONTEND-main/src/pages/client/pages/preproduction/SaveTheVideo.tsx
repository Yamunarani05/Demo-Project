...
import { ExternalLink, Send, Video as VideoIcon, Link as LinkIcon, Check, AlertCircle } from 'lucide-react';
...
export default function SaveTheVideo() {
    ...
    useEffect(() => {
        const fetchLinks = async () => {
            try {
                ...
                const res = await axios.get(`${API_URL}/preproduction/raw-data-links?type=Save%20the%20Video`, {
                ...
                if (res.data?.success) {
                    ...
                    if (res.data.data?.requirements) {
                        ...
                        if (reqs.referenceLink) {
                            setReferenceLink(reqs.referenceLink);
                        }
                        if (reqs.imageNumbers) {
                            setVideoNumbers(reqs.imageNumbers);
                        }
                    }
    ...
    const handleApproveFinalDelivery = async () => {
        try {
            ...
            if (finalDelivery) {
                setFinalDelivery({ ...finalDelivery, notes: (finalDelivery.notes || '') + ' [Save the Video_Final_Approved]' });
            }
        } catch (err: any) {
    ...
    const handleRejectFinalDelivery = async () => {
        ...
        try {
            ...
            setRejectQuery('');
            if (finalDelivery) {
                setFinalDelivery({ ...finalDelivery, notes: (finalDelivery.notes || '') + ' [Save the Video_Rejected]' });
            }
        } catch (err: any) {
            ...
            {finalDelivery ? (
                <div className="bg-white rounded-2xl border border-indigo-200 shadow-sm p-6 flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                        {(finalDelivery.videoDriveLink || finalDelivery.driveLink) ? (
                            <a href={(finalDelivery.videoDriveLink || finalDelivery.driveLink) as string} target="_blank" rel="noreferrer" className="group flex items-center justify-between p-4 rounded-xl bg-pink-50 border border-pink-100 hover:bg-pink-100 hover:border-pink-200 transition-all text-pink-700 font-medium text-sm">
                                <div className="flex items-center gap-3 truncate">
                                    <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm shrink-0">
                                        <ExternalLink size={20} className="text-pink-600" />
                                    </div>
                                    <span className="truncate text-base">Save the Video Delivery Link</span>
                                </div>
                                <ExternalLink size={18} className="text-pink-400 group-hover:text-pink-600 transition-colors" />
                            </a>
                        ) : null}
                    </div>

                    {finalDelivery.notes?.includes('[Save the Video_Final_Approved]') ? (
                    ...
                    ) : finalDelivery.notes?.includes('[Save the Video_Rejected]') ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col relative overflow-hidden">
                    ...
                    <p className="text-sm text-slate-500 mb-6 flex-1">
                        Your production team has uploaded the raw media. Please review them using the secure links below.
                    </p>
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                        ...
                        {videoDriveLink ? (
                            <a href={videoDriveLink} target="_blank" rel="noreferrer" className="group flex items-center justify-between p-3.5 rounded-xl bg-pink-50 border border-pink-100 hover:bg-pink-100 hover:border-pink-200 transition-all text-pink-700 font-medium text-sm">
                                <div className="flex items-center gap-3 truncate">
                                    <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm shrink-0">
                                        <VideoIcon size={16} className="text-pink-600" />
                                    </div>
                    ...
                    {!isRawDataApproved ? (
                        <div className="mt-4 flex flex-col items-center justify-center p-4 rounded-xl bg-slate-50 border border-slate-200 border-dashed gap-3">
                            <p className="text-sm text-slate-500 text-center">Please review the raw video clips and approve to proceed to the next steps.</p>
                            <button 
                ...
                <div className={`bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col relative overflow-hidden transition-all ${!isRawDataApproved ? 'opacity-60 grayscale-[0.5] pointer-events-none' : ''}`}>
                    <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
                        <LinkIcon size={100} />
                    </div>
                    ...
                    <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6">
                            ...
                            <p className="text-sm text-slate-500 mb-6">
                                Please let us know what changes are required for the Save the Video delivery. Be as specific as possible.
                            </p>
                            <textarea 
                                value={rejectQuery}
                                onChange={(e) => setRejectQuery(e.target.value)}
                                placeholder="E.g., The color grading seems too dark, and please change the background music..."
