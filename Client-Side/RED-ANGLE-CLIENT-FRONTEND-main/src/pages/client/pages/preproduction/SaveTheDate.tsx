...
import { ExternalLink, Send, Image as ImageIcon, Link as LinkIcon, Check, AlertCircle } from 'lucide-react';
...
export default function SaveTheDate() {
    ...
    useEffect(() => {
        const fetchLinks = async () => {
            try {
                ...
                const res = await axios.get(`${API_URL}/preproduction/raw-data-links?type=Save%20the%20Date`, {
    ...
    const handleApproveFinalDelivery = async () => {
        try {
            ...
            if (finalDelivery) {
                setFinalDelivery({ ...finalDelivery, notes: (finalDelivery.notes || '') + ' [Save the Date_Final_Approved]' });
            }
        } catch (err: any) {
    ...
    const handleRejectFinalDelivery = async () => {
        ...
        try {
            ...
            setRejectQuery('');
            if (finalDelivery) {
                setFinalDelivery({ ...finalDelivery, notes: (finalDelivery.notes || '') + ' [Save the Date_Rejected]' });
            }
        } catch (err: any) {
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            ...
            {finalDelivery ? (
                <div className="bg-white rounded-2xl border border-indigo-200 shadow-sm p-6 flex flex-col relative overflow-hidden">
                        ...
                        {finalDelivery.driveLink ? (
                            <a href={finalDelivery.driveLink} target="_blank" rel="noreferrer" className="group flex items-center justify-between p-4 rounded-xl bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 hover:border-indigo-200 transition-all text-indigo-700 font-medium text-sm">
                                <div className="flex items-center gap-3 truncate">
                                    ...
                                    <span className="truncate text-base">Save the Date Delivery Link</span>
                                </div>
                    ...
                    {finalDelivery.notes?.includes('[Save the Date_Final_Approved]') ? (
                    ...
                    ) : finalDelivery.notes?.includes('[Save the Date_Rejected]') ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col relative overflow-hidden">
                    ...
                    {!isRawDataApproved ? (
                        <div className="mt-4 flex flex-col items-center justify-center p-4 rounded-xl bg-slate-50 border border-slate-200 border-dashed gap-3">
                            <p className="text-sm text-slate-500 text-center">Please review the raw data and approve to proceed to the next steps.</p>
                            <button 
                    ...
                    <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6">
                            ...
                            <p className="text-sm text-slate-500 mb-6">
                                Please let us know what changes are required for the Save the Date delivery. Be as specific as possible.
                            </p>
                            <textarea 
                                value={rejectQuery}
