...
export default function ClientTopbar({ toggleMenu }: { toggleMenu?: () => void }) {
    const userStr = localStorage.getItem('ra_user')
    const user = userStr ? JSON.parse(userStr) : null

    return (
        <header className="h-[var(--topbar-height)] bg-white/80 backdrop-blur-md border-b border-slate-200/80 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30 shadow-sm transition-all duration-300">
            {/* Left side (Hamburger & Title) */}
            <div className="flex items-center gap-3">
                <button 
                    onClick={toggleMenu} 
                    className="md:hidden p-1.5 -ml-1.5 text-slate-500 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-colors"
                >
                    <Menu size={22} />
                </button>
