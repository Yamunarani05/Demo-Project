...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 400) {
      console.error("Bad Request:", error.response.data?.message || error.response.data);
    }
...
export const formatErrorMessage = (error: any): string => {
  ...
  if (error.response) {
    ...
    if (data?.message) return `${status} Error: ${data.message}`;
