...
import { memoryStore, StudioRecord } from '../models/db';
...
router.put('/:id/status', async (req: Request, res: Response) => {
  ...
  if (status === 'active' || status === 'approved') {
    const { sendApprovalEmail } = await import('../services/email/emailService');
    emailResult = await sendApprovalEmail(emailData);
  } else if (status === 'rejected') {
    const { sendRejectionEmail } = await import('../services/email/emailService');
