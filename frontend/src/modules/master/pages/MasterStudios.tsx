...
export default function MasterStudios() {
  const { studiosList, approveStudio, rejectStudio, requestPayment } = useAuth();
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h1 className="text-2xl font-black text-slate-900 font-display tracking-tight">
            Studios Directory ({studiosList.length} Studios Total)
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Monitor client counts, staff numbers, 7-day trial status, and Razorpay payment records across all studios.
          </p>
        </div>
      ...
      <motion.div
        variants={itemFadeSlide}
        className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center gap-3"
      >
          ...
          {filteredStudios.map((studio, idx) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => !isPending && navigate(location.pathname.startsWith('/great-master') ? `/great-master/masters/${studio.id}` : `/master/studios/${studio.id}`)}
                className={`rounded-2xl border p-6 shadow-xs hover:shadow-md transition-all cursor-pointer flex flex-col justify-between group ${
                  isPending
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      {isPending ? (
                        ...
                        <span className="px-3 py-1 rounded-full bg-rose-50 text-rose-700 text-xs font-extrabold border border-rose-200 flex items-center gap-1 shadow-xs">
                          <Clock className="w-3.5 h-3.5 text-rose-600" />
                          <span>Trial Expired</span>
                        </span>
                      ) : isConverted ? (
                        <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-extrabold border border-emerald-200 flex items-center gap-1 shadow-xs">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                          <span>Paid & Active</span>
                        </span>
                      ) : (
                        <span className="px-3 py-1 rounded-full bg-purple-50 text-purple-700 text-xs font-bold border border-purple-200 flex items-center gap-1.5 shadow-xs">
                ...
                {isPending ? (
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      {(isExpired || studio.paymentStatus === 'PAYMENT_PENDING' || studio.paymentStatus === 'PENDING') && !isConverted && (
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); requestPayment(studio.id); }}
                          className="py-1.5 px-3 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs transition-colors flex items-center gap-1 cursor-pointer shadow-xs"
                        >
                      ...
                      {!isConverted && (
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); setSelectedCheckoutStudio(studio); setIsCheckoutOpen(true); }}
                          className="py-1.5 px-3 rounded-lg bg-[#5E35B1] hover:bg-[#512DA8] text-white font-bold text-xs transition-colors flex items-center gap-1 cursor-pointer shadow-xs"
                        >
                    ...
                    <div className="text-xs font-semibold text-purple-700 flex items-center gap-1 ml-auto">
                      <span>Details</span>
