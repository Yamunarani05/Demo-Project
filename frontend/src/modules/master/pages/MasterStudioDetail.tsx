  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemFadeSlide} className="flex items-center justify-between">
        <Link
          to={location.pathname.startsWith('/great-master') ? '/great-master/masters' : '/master/studios'}
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-xs text-slate-500 mt-2 flex-wrap">
                ...
                <span>·</span>
                <span>Admin / Lead: <strong className="text-slate-700">{studio.adminName}</strong></span>
                <span>·</span>
                <span className="text-purple-600 font-medium">{studio.adminEmail}</span>
              </div>
            </div>
          </div>

          {/* Action CTA: Enter Studio Workspace */}
          <div className="flex items-center gap-3">
            <button
              onClick={() => {
                if (typeof (window as any).switchStudio === 'function') {
                  (window as any).switchStudio(studio.id);
                }
                localStorage.setItem('demo_active_studio_id', studio.id);
                navigate(`/master/dashboard?studioId=${studio.id}`);
              }}
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold text-xs sm:text-sm shadow-md shadow-purple-900/20 flex items-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
            >
              <span>Enter Studio Operations Workspace</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>

        {/* 5 Core Master Overview Metrics Required for Great Master Oversight */}
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-6 pt-6 border-t border-slate-100">
          <motion.div whileHover={{ y: -3 }} className="p-3.5 bg-purple-50/60 rounded-xl border border-purple-100 text-center transition-all">
            ...
            <div className="text-2xl font-black text-purple-800 mt-1 font-display">
              <AnimatedCounter end={studioClients.length || studio.onboardedClientsCount || 5} duration={600} />
            </div>
            <div className="text-[10px] text-purple-600 mt-0.5">Active studio clients</div>
          </motion.div>

          <motion.div whileHover={{ y: -3 }} className="p-3.5 bg-indigo-50/60 rounded-xl border border-indigo-100 text-center transition-all">
            <div className="text-[11px] font-bold text-indigo-900 flex items-center justify-center gap-1">
              <FolderKanban className="w-3.5 h-3.5 text-indigo-600" />
              <span>Project Count</span>
            </div>
            <div className="text-2xl font-black text-indigo-800 mt-1 font-display">
              <AnimatedCounter end={studioClients.length * 2 || 10} duration={600} />
            </div>
            <div className="text-[10px] text-indigo-600 mt-0.5">Pre & post wedding</div>
          </motion.div>

          <motion.div whileHover={{ y: -3 }} className="p-3.5 bg-emerald-50/60 rounded-xl border border-emerald-100 text-center transition-all">
            ...
            <div className="text-2xl font-black text-emerald-800 mt-1 font-display">
              ₹<AnimatedCounter end={studioClients.reduce((acc, c) => acc + (c.budget || 75000), 0) / 1000} duration={600} />k
            </div>
            <div className="text-[10px] text-emerald-600 mt-0.5">Total booked value</div>
          </motion.div>

          <motion.div whileHover={{ y: -3 }} className="p-3.5 bg-amber-50/60 rounded-xl border border-amber-100 text-center transition-all">
            ...
            <div className="text-2xl font-black text-amber-800 mt-1 font-display">
              <AnimatedCounter end={8} duration={600} />
            </div>
            <div className="text-[10px] text-amber-600 mt-0.5">6 converted · 2 active</div>
          </motion.div>

          <motion.div whileHover={{ y: -3 }} className="p-3.5 bg-blue-50/60 rounded-xl border border-blue-100 text-center transition-all col-span-2 sm:col-span-1">
            <div className="text-[11px] font-bold text-blue-900 flex items-center justify-center gap-1">
              <Camera className="w-3.5 h-3.5 text-blue-600" />
              <span>Production Summary</span>
            </div>
            <div className="text-2xl font-black text-blue-800 mt-1 font-display">
              <AnimatedCounter end={4} duration={600} />
            </div>
            <div className="text-[10px] text-blue-600 mt-0.5">3 in shoot · 1 delivery</div>
          </motion.div>
        </div>
      </motion.div>
