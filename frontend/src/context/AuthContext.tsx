...
export type UserRole = 'great_master' | 'super_admin' | 'studio_admin';
...
interface AuthContextType {
  ...
  requestPayment: (studioId: string) => Promise<void>;
  createRazorpayOrder: (studioId: string, amount?: number, planName?: string) => Promise<any>;
  verifyRazorpayPayment: (data: { studioId: string; razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature?: string }) => Promise<any>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const hasPending = parsed.some((s: DemoStudio) => s.status === 'pending');
          if (!hasPending) {
            const defaultPending = INITIAL_STUDIOS.filter((s) => s.status === 'pending');
            if (defaultPending.length > 0) {
              return [...parsed, ...defaultPending];
            }
          }
          return parsed;
        }
  ...
  // 7. Email logs & real-time sync with backend
  ...
  const refreshDashboardData = async () => {
    try {
      ...
      if (res && res.success && res.data) {
        const backendStudios = res.data.ownerDashboard?.masterTable || res.data.masterTable || res.data.studioSummaries;
        if (Array.isArray(backendStudios) && backendStudios.length > 0) {
          setStudiosList((prev) => {
            const updated = [...prev];
            backendStudios.forEach((bs: any) => {
              const idx = updated.findIndex((s) => s.id === bs.id);
              if (idx >= 0) {
    ...
    try {
      const emailRes = await api.getEmailHistory();
  ...
  // Real-time periodic polling (every 5 seconds)
  useEffect(() => {
  ...
  const approveStudio = async (studioId: string) => {
    const now = new Date();
    const trialEndDate = new Date(now.getTime() + 7 * 86400000);
    let emailSuccess = true;
    let apiData: any = null;

    try {
      const res = await api.approveMasterRequest(studioId);
      if (res && res.success) {
        apiData = res.data;
        if (res.emailSent === false) emailSuccess = false;
      }
    } catch (e) {}
    ...
    toast.success('7-Day Free Trial Approved!', {
      description: `Approval email ${emailSuccess ? 'sent' : 'queued'} to ${studio?.adminEmail || 'studio admin'}.`,
    });
    ...
    if (studio) {
      const activity: DemoActivity = {
        ...
        action: 'Free Trial Approved (7 Days)',
        details: `${studio.name} 7-day free trial approved by Master Admin. Approval email sent to ${studio.adminEmail}.`,
        timeAgo: 'Just now',
        type: 'status_update',
      };
  ...
  const rejectStudio = async (studioId: string, reason?: string) => {
    let emailSuccess = true;
    try {
      const res = await api.rejectMasterRequest(studioId, reason);
      if (res && res.emailSent === false) emailSuccess = false;
    } catch (e) {}
    ...
    toast.info('Studio access request rejected.', {
      description: `Rejection email ${emailSuccess ? 'sent' : 'queued'} to ${studio?.adminEmail || 'studio admin'}.`,
    });
    ...
    if (studio) {
      const activity: DemoActivity = {
        ...
        details: `${studio.name} request was rejected by Master Admin.`,
        timeAgo: 'Just now',
        type: 'status_update',
      };
      setActivitiesList((prev) => [activity, ...prev]);
  ...
  const requestPayment = async (studioId: string) => {
    let emailSuccess = true;
    try {
      const res = await api.requestPayment(studioId);
      if (res && res.emailSent === false) emailSuccess = false;
    } catch (e) {}
    ...
    toast.success('Payment Requested!', {
      description: `Payment request email ${emailSuccess ? 'sent' : 'queued'} to ${studio?.adminEmail || 'studio admin'}.`,
    });

    refreshDashboardData();
  };

  const createRazorpayOrder = async (studioId: string, amount?: number, planName?: string) => {
    try {
      const res = await api.createRazorpayOrder({ studioId, amount, planName });
      return res;
    } catch (e: any) {
      toast.error('Failed to create Razorpay order: ' + (e?.message || ''));
      throw e;
    }
  };

  const verifyRazorpayPayment = async (data: { studioId: string; razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature?: string }) => {
    try {
      const res = await api.verifyRazorpayPayment(data);
      if (res && res.success) {
        setStudiosList((prev) =>
          prev.map((s) => {
            if (s.id !== data.studioId) return s;
            return {
              ...s,
              status: 'active',
              trialStatus: 'CONVERTED',
              paymentStatus: 'PAYMENT_SUCCESS',
              totalRevenue: (s.totalRevenue || 0) + (s.amount || 4999),
            };
          })
        );
        toast.success('Payment Verified!', {
          description: `Razorpay payment verified (ID: ${data.razorpay_payment_id}). Confirmation email sent.`,
        });
      }
      refreshDashboardData();
      return res;
    } catch (e: any) {
      toast.error('Razorpay payment verification failed: ' + (e?.message || ''));
      throw e;
    }
  };
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        isGreatMaster: user?.role === 'great_master' || user?.role === 'super_admin',
