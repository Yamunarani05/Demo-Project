  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome back, ${result.user.name}!`, {
        description:
          result.user.role === 'great_master' || result.user.role === 'super_admin'
            ? 'Logged in to Great Master Platform Overview.'
            : `Logged in to ${result.studio?.name || 'Studio'} Workspace.`,
      });

      if (result.user.role === 'great_master' || result.user.role === 'super_admin') {
        navigate('/great-master/dashboard');
      } else {
