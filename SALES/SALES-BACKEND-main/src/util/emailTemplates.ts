...
export const createApprovalEmailContent = (
  ...
  return `
    ...
    <div class="highlight-box">
        ...
        <p><strong>Rent Type:</strong> ${rentType === "SHORT_TERM" ? "Short Term" : "Long Term"
    }</p>
        ${roomNo
      ? `<p><strong>Room Number:</strong> ${roomNo}</p>`
      : "<p><strong>Room:</strong> Will be assigned soon</p>"
    }
        ${rentType === "LONG_TERM" && roomNo && rentAmount
      ? `<p><strong>Monthly Rent:</strong> ${formatCurrency(
        rentAmount
      )}</p>`
      : ""
    }
        ${rentType === "SHORT_TERM" && pricePerDay
      ? `<p><strong>Daily Rate:</strong> ${formatCurrency(
        pricePerDay
      )}</p>`
      : ""
    }
        ${advanceAmount
      ? `<p><strong>Advance Amount:</strong> ${formatCurrency(
        advanceAmount
      )}</p>`
      : ""
    }
        <p><strong>Date of Joining:</strong> ${formatDate(dateOfJoining)}</p>
        ${rentType === "SHORT_TERM" && dateOfRelieving
      ? `<p><strong>Stay Until:</strong> ${formatDate(dateOfRelieving)}</p>`
      : ""
    }
    ...
    <div class="highlight-box" style="background-color: #fff3cd; border-left-color: #ffc107;">
        ...
        <ul style="margin-bottom: 0;">
            ${advanceAmount
      ? `<li><strong>Advance Payment:</strong> ${formatCurrency(
        advanceAmount
      )} (to be paid before check-in)</li>`
      : ""
    }
            ${rentType === "LONG_TERM" && roomNo && rentAmount
      ? `<li><strong>Monthly Rent:</strong> ${formatCurrency(
        rentAmount
      )} (due on the same date each month as your joining date)</li>`
      : ""
    }
            ${rentType === "SHORT_TERM" && shortTermTotal > 0
      ? `<li><strong>Total Amount for Stay:</strong> ${formatCurrency(
        shortTermTotal
      )} (already paid and approved)</li>`
      : ""
    }
    ...
    <div class="highlight-box" style="background-color: #e7f3ff; border-left-color: #007bff;">
        ...
        <ul style="margin-bottom: 0;">
            ...
            ${rentType === "SHORT_TERM"
      ? `<li><strong>Check-out Date:</strong> ${dateOfRelieving ? formatDate(dateOfRelieving) : "To be confirmed"
      }</li>
    ...
    <div class="highlight-box" style="background-color: #d4edda; border-left-color: #28a745;">
        ...
        <div style="background: #e7f3ff; border-left: 4px solid #007bff; padding: 15px; margin: 15px 0;">
          <p style="margin: 0; color: #004085; font-size: 14px;">
            ...
            2. Enter your email: <strong>${memberName
        .toLowerCase()
        .replace(/\s+/g, "")}@email.com</strong> (use your registered email)<br>
...
export const createRejectionEmailContent = (
  memberName: string,
  pgType: string
) => {
  return `
...
export const createPasswordResetEmailContent = (
  email: string,
  otpCode: string,
  resetLink: string
) => {
  return `
    ...
    <p>
      We received a request to reset your password for your <strong>Red Angle</strong> account.
    </p>

    <p>
      <strong>🔑 Your Password Reset OTP</strong><br />
    ...
    <p><strong>⚠️ This OTP expires in 15 minutes.</strong></p>
  `;
};

// Welcome email template for new registrations
export const createWelcomeEmailContent = (
  userName: string,
  tempPassword?: string
) => {
  return `
...
export const createPaymentConfirmationEmailContent = (
  ...
  return `
    ...
    <div class="highlight-box" style="background-color: #d4edda; border-left-color: #28a745;">
        ...
        ${month && year
      ? `<p><strong>For Period:</strong> ${getMonthName(month)} ${year}</p>`
      : ""
    }
    ...
    <div class="highlight-box" style="background-color: #e7f3ff; border-left-color: #007bff;">
      ...
      ? `Your next monthly rent payment will be due on the same date next month (${getMonthName(
        month === 12 ? 1 : month + 1
...
// Quotation wrapper template
export const quotationApprovalEmailTemplate = (
  content: string,
  subject: string
) => {
  return `
...
// Customer Quotation Welcome Email Template
export const createQuotationWelcomeEmailContent = (
  customer: any,
  quotation: any,
  quotationToken: string,
  notes?: string,
  clientSetupUrl?: string
) => {
  const quotationViewUrl =
    `${ENV.SALES_FRONTEND_URL.replace(/\/+$/, "")}/quotation/view?token=${quotationToken}`;
  console.log("ENV.SALES_FRONTEND_URL =", ENV.SALES_FRONTEND_URL);
  console.log("quotationViewUrl =", quotationViewUrl);

  const formatPaymentTerms = (terms: number | null | undefined) => {
    switch (terms) {
      case 1: return "Advance";
      case 2: return "On the Spot 80% on Event Day";
      case 3: return "On the Spot 40%";
      case 4: return "Remaining 20%";
      default: return terms ? String(terms) : "";
    }
  };
  ...
  return `
    ...
    <p>Thank you for choosing us for your ${customer.eventType}! We're excited to work with you to make your special day unforgettable.</p>
    ...
    <h3 style="color: #667eea;">Quotation Made For You</h3>
    
    <div style="background-color: #f8f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #e0e0ff;">
      <h4 style="margin-top: 0; color: #764ba2;">${quotation.serviceName}</h4>
      <p style="margin: 10px 0;"><strong>Description:</strong> ${quotation.description || "N/A"
    }</p>
      <p style="margin: 10px 0;"><strong>Quantity:</strong> ${quotation.quantity
    }</p>
      <p style="margin: 10px 0; font-size: 18px; color: #667eea;"><strong>Price:</strong> ₹${quotation.price
    }</p>
      ${quotation.combo
      ? `<p style="margin: 10px 0;"><strong>Package:</strong> ${quotation.combo}</p>`
      : ""
    }
      ${quotation.terms
      ? `<p style="margin: 10px 0;"><strong>Terms:</strong> ${formatPaymentTerms(quotation.terms)}</p>`
      : ""
    }
      ${notes
      ? `<div style="margin-top: 15px; padding: 12px; background-color: #fffaf0; border-left: 4px solid #ffc107; border-radius: 4px;">
        <h5 style="margin: 0 0 8px 0; color: #856404;">Additional Notes</h5>
        <p style="margin: 0; color: #5c4b1f;">${notes}</p>
      </div>`
      : ""
    }
      
      ${quotation.items && quotation.items.length > 0
      ? `<div style="margin-top: 20px; border-top: 1px solid #e0e0ff; padding-top: 15px;">
              <h5 style="margin: 0 0 10px 0; color: #764ba2;">Add-ons Included:</h5>
              <ul style="list-style-type: none; padding-left: 0; margin: 0;">
                ${quotation.items.map((item: any) => `
                  <li style="padding: 8px 0; border-bottom: 1px dashed #e0e0ff; display: flex; justify-content: space-between;">
                    <span>${item.name} ${item.category ? `(${item.category})` : ""} ${item.quantity > 1 ? ` × ${item.quantity}` : ""}</span>
                    <span style="font-weight: bold; color: #667eea;">${item.price ? "₹" + item.price : "Included"}</span>
                  </li>
                `).join('')}
              </ul>
            </div>`
  ...
  <a href="${clientSetupUrl}" 
     style="
        ...
        margin-bottom: 10px;
     ">
     ${clientSetupUrl.includes('set-password') ? 'Access Client Portal & Set Password' : 'Access Client Portal'}
  </a>
  ` : ''}
</div>
    <p style="color: #666; font-size: 14px; text-align: center;">
      If you have any questions about this quotation, please reach out to us directly at your earliest convenience.
    </p>
  `;
};
...
export const createAdminInvoiceEmailContent = (
  clientName: string,
  invoiceNumber: string,
  token: string,
  markings?: { label: string; value: string }[]
) => {

  const invoiceUrl = `${ENV.SALES_FRONTEND_URL}/invoice/${token}`;

  return `
    <h2>📄 Invoice from ${ENV.COMPANY_NAME}</h2>

    <p>Dear ${clientName},</p>

    <p>Your invoice has been generated. Please review your event and payment details below before viewing the full invoice.</p>

    ${markings && markings.length > 0
      ? `
      <div style="margin-top: 30px; padding: 20px; background-color: #f8f9ff; border-left: 4px solid #4f46e5; border-radius: 4px;">
        <h3 style="margin-top: 0; margin-bottom: 15px; color: #4f46e5;">Event & Payment Details</h3>
        ...
        .map(
          (m) => `
              <tr>
                <td style="padding: 8px 0; border-bottom: 1px solid #e0e0ff; color: #333; font-weight: bold; width: 40%;">${m.label || ""
            }</td>
                <td style="padding: 8px 0; border-bottom: 1px solid #e0e0ff; color: #555;">${m.value || "-"
            }</td>
              </tr>
            `
        )
...
// Generate Client Setup Welcome Email Template
export const createClientSetupEmailContent = (
  userName: string,
  setupLink: string
) => {
  return `
    ...
    <p>We have successfully established your client portal profile! Welcome to the ${ENV.COMPANY_NAME} family.</p>
    
    <div class="highlight-box">
        <h3 style="margin-top: 0; color: #28a745;">✅ Action Required: Secure Your Account</h3>
        <p>Before you can access your dashboard to view invoices, trackers, and quotations, you must set an access password.</p>
    </div>
    
    <div style="text-align: center; margin: 30px 0;">
        <a href="${setupLink}" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 14px 40px; text-decoration: none; border-radius: 30px; font-weight: bold; font-size: 16px; box-shadow: 0 4px 15px rgba(102,126,234,0.4);">
            Set Your Password
        </a>
    </div>
    ...
    <div class="highlight-box" style="background-color: #fff3cd; border-left-color: #ffc107;">
        <h4 style="margin-top: 0; color: #856404;">🔒 Security Reminder</h4>
        <ul style="margin-bottom: 0;">
            <li>Do not share this setup link with anyone.</li>
            <li>Choose a strong password with at least 6 characters.</li>
        </ul>
    </div>
    
    <p>We're committed to providing you with the best possible experience. If you have any questions, our support team is always here to help.</p>
