...
async getAll(
  ...
  async sendToClient(req: Request, res: Response) {
    try {
      ...
      console.log("Quotation Lead created:", quotationLead);
      ...
      // Respond immediately — email/quotation lookups below are optional
      res.status(201).json({
        success: true,
      ...
      console.error("Error sending quotation to client:", err);
      res.status(400).json({
        success: false,
