...
import { syncLeadToPrePostDb } from "../services/prePostLeadSyncService";
...
class InvoiceController {
  ...
  async sendInvoiceToClient(req: Request, res: Response) {
    ...
    try {
      ...
      const invoiceNumber = `INV-${new Date().getFullYear()}-${String(invoiceId).padStart(6, "0")}`;
    ...
    try {
      ...
      invoices.forEach((inv) => {
        const row = sheet.addRow([
          `INV-${String(inv.invoiceId).padStart(6, "0")}`,
          `${inv.lead.firstName ?? ""} ${inv.lead.lastName ?? ""}`.trim() || "-",
          inv.lead.email ?? "-",
  ...
  async updatePreview(req: Request, res: Response) {
    try {
      ...
      if (billNo !== undefined && updated.leadId) {
        const updatedLead = await prisma.leadsDetail.update({
          where: { leadId: updated.leadId },
          data: { leadSerialNumber: billNo },
        });
        try {
          await syncLeadToPrePostDb(updatedLead as any);
        } catch (syncError) {
          console.error("Failed to sync lead to PrePost DB (Non-fatal):", syncError);
        }
      }

      if (previewItems !== undefined) {
        // Use executeRaw because Prisma Client might not be regenerated yet
