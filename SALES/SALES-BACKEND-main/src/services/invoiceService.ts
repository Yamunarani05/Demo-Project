...
class InvoiceService {
  ...
  async createInvoice(data: CreateInvoicePayload, userId: number) {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        // Find the quotation lead to pull the discount
    ...
    try {
      const whereCondition: any = {
        AND: [
          {
            OR: [
              { currentStage: { in: ["Confirmation", "Finalised", "callUp"] } },
              { invoices: { some: {} } }
            ]
          }
        ]
  ...
  async getInvoiceById(invoiceId: number) {
    try {
      ...
      if (!invoice) throw new Error("Invoice not found");
      console.log("ADDONS FROM DB:", invoice.addons);
      ...
      const totalPrice = packageTotal + addonTotal + itemsTotal;
      const balance = (totalPrice - discount) - paid;

      const itemsByCategory: Record<string, any[]> = buildInvoiceViewModel(invoice);
      console.log("Returning invoice paid:", invoice.paid);
