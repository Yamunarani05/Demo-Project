...
let invoice: any = existingInvoice;
...
if (!invoice) {
  ...
  if (!quotation || !quotation.packageId) {
    throw new Error("Quotation packageId missing");
  }
  ...
  invoice = await prisma.invoices.create({
    data: {
      ...
      plan: "Standard",
      status: "Pending",
      discount: quotationLead.discount ? Number(quotationLead.discount) : 0,
      packageInvoices: {
        create: [
          {
...
console.log("Addons found:", addons);
...
await notificationService.createUserNotification({
  ...
  title:
    status === "approved"
      ? "Quotation Approved"
      : "Quotation Rejected",
  message: `Quotation for lead ${lead?.firstName ?? ""} ${
    lead?.lastName ?? ""
