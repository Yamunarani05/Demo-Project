...
async function request(endpoint: string, options: RequestInit = {}) {
  ...
  console.log(`🌐 API Request to port 5000: ${options.method || 'GET'} ${url}`);
  ...
  try {
    ...
    console.log(`📥 Response from port 5000: ${response.status} ${response.statusText}`);
    ...
    console.log('✅ Response data received from port 5000');
    return data;

  } catch (error: any) {
    console.error('❌ API Error connecting to port 5000:', error);

    if (error.message.includes('Failed to fetch')) {
      throw new Error(`Cannot connect to backend at http://localhost:5000. Please ensure backend is running on port 5000.`);
    }
...
export async function listLeadsPaginated({ 
  ...
  console.log('📊 Fetching leads from port 5000:', { page, limit, search });
  ...
  try {
    ...
    console.log('📦 Raw response from backend (port 5000):', response);
    ...
    console.error('❌ Error fetching from port 5000:', error);
    throw error;
  }
}
...
export async function getPartnerAssignedLeads() {
  console.log('👤 Partner: fetching assigned leads from port 5000');
...
export async function createLead(data: Partial<Lead>) {
  console.log('➕ Creating lead on port 5000:', data);
...
export async function getMonthWiseLeads(startDate?: string, endDate?: string): Promise<MonthWiseLeads[]> {
  console.log('📈 Fetching month-wise leads for graph from port 5000');
...
export async function updateLead(id: string | number, data: Partial<Lead>) {
  console.log(`✏️ Updating lead ${id} on port 5000:`, data);
