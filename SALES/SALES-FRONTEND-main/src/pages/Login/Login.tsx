...
const Login = () => {
  ...
  // Check URL parameters for SSO handoff from DEMO-UI or external portals
  useEffect(() => {
    ...
    if (urlToken && urlRole) {
      ...
      } else if (urlRole === "employee") {
        navigate("/employee/employee-profile", { replace: true });
      } else if (urlRole === "partner") {
        navigate("/partner/dashboard", { replace: true });
      }
      return;
    }

    // Auto redirect if already logged in with valid token and role
    const existingToken = localStorage.getItem("token");
    const existingRole = localStorage.getItem("role");
    if (existingToken && existingRole) {
      if (existingRole === "admin") navigate("/admin/dashboard", { replace: true });
      else if (existingRole === "employee") navigate("/employee/employee-profile", { replace: true });
      else if (existingRole === "partner") navigate("/partner/dashboard", { replace: true });
    }
  }, [navigate]);
