...
const Employees = () => {
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    const filterLow = positionFilter.toLowerCase();
    const pos = (emp.position || '').toLowerCase();
    const role = (emp.user?.role || '').toLowerCase();
    const salesType = (emp.salesType || '').toLowerCase();
    return pos.includes(filterLow) || role.includes(filterLow) || salesType.includes(filterLow);
  });
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">
                Sales Team Directory ({totalEmployees})
              </h1>
              <p className="text-xs text-gray-500 mt-0.5">Sales representatives, telecallers & channel partners</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {positionFilter === 'All' ? 'Filter Role' : positionFilter}
                </button>

                {filterOpen && (
                  <div className="absolute right-0 mt-1 w-44 bg-white border rounded-lg shadow z-10 text-xs">
                    {['All', 'Executive', 'Telecaller', 'Manager', 'Partner'].map((pos) => (
                      <button
                        key={pos}
