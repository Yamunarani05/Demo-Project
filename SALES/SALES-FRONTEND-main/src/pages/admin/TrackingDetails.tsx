  ...
  useEffect(() => {
    const fetchLeadsAndInvoices = async () => {
      ...
      try {
        ...
        const formattedLeads: LeadTracking[] = leadsData.map((lead: any) => {
          const employee = lead.leadEmployee?.[0]?.employee;
          const employeeName = employee
            ? `${employee.firstName} ${employee.lastName}`
            : 'Unassigned';

          const status = ['Confirmation', 'Finalised'].includes(lead.currentStage)
            ? 'Paid'
            : 'Unpaid';

          const leadInvoice = invoicesData.find((inv: any) => inv.leadId === lead.leadId);
          const invoiceId = leadInvoice?.invoices?.[0]?.invoiceId
            ? `INV${leadInvoice.invoices[0].invoiceId}`
            : '-';

          return {
            id: String(lead.leadId),
            leadId: lead.leadSerialNumber || `LD${lead.leadId}`,
            leadName: `${lead.firstName} (${lead.eventType})`,
            contactId: lead.contactNumber,
            invoiceId,
            email: lead.email,
            billingDate: leadInvoice?.invoices?.[0]?.billingDate
              ? new Date(leadInvoice.invoices[0].billingDate).toLocaleDateString('en-GB', {
                day: '2-digit',
                month: 'short',
                year: 'numeric',
              })
            ...
            eventDate: new Date(lead.eventDate).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
              year: 'numeric',
            }),
            createdTime: new Date(lead.createdTime).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
              year: 'numeric',
            }),
            ...
            plan: `${lead.eventType} Plan`,
            status,
            currentStage: lead.currentStage,
          };
        });

        setLeadTracking(formattedLeads);
      } catch (err) {
                  ...
                  <div className="inline-flex items-center gap-4 rounded-full bg-white shadow-md border border-gray-200 px-4 py-2 text-xs">
                    ...
                    <button
                      ...
                      className={`rounded-full p-1 transition ${leadCurrentPage === 1
                        ...
                        : 'text-gray-400 hover:text-[#6938ef]'
                        }`}
                    >
                          ...
                          {emp.position ? (
                            <div className="flex items-center gap-2">
                              ...
                              {emp.position === 'Copywriter' && (
                                <span className="px-1.5 py-0.5 bg-purple-100 text-purple-800 rounded-full text-[10px] font-semibold">
                                  Senior
                                </span>
                              )}
                            </div>
                ...
                <div className="flex justify-end px-6 py-3 bg-transparent">
                  <div className="inline-flex items-center gap-4 rounded-full bg-white shadow-md border border-gray-200 px-4 py-2 text-xs">
                    <span className="text-gray-900">
                      ...
                      {Math.min(employeeEndIndex, filteredEmployees.length)} of{' '}
