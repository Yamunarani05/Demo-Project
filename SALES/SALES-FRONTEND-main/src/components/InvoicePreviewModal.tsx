...
const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({
  ...
  const [billNoValue, setBillNoValue] = React.useState<string>(invoice?.billNo ?? invoice?.invoiceId?.toString() ?? "");
  ...
  React.useEffect(() => {
    if (isOpen) {
      setBillNoValue(invoice?.billNo ?? invoice?.invoiceId?.toString() ?? "");
  ...
  const balance = React.useMemo(() => {
    if (qtyOverrides["BALANCE_OVERRIDE"] !== undefined) return qtyOverrides["BALANCE_OVERRIDE"];
    return overall - paidValue;
  }, [overall, paidValue, qtyOverrides]);
      ...
      api.get(`/invoices/${invoice.invoiceId}/markings`)
        .then(res => {
          ...
          if (data.length > 0) {
            ...
            // Default rows if no markings saved yet
            setEvents([
              { title: "Event Name", value: invoice.eventName ?? "" },
              { title: "DATE", value: formatDate(invoice.billingDate) },
            ]);
                  ...
                  <tr className="inv-totals-row">
                    ...
                    <td className="t-amount">
                      ...
                      <input
                        type="text"
                        data-html2canvas-ignore="true"
                        value={balanceFocused ? (qtyOverrides["BALANCE_OVERRIDE"] !== undefined ? qtyOverrides["BALANCE_OVERRIDE"] : balance) : formatCurrency(balance)}
                        onFocus={(e) => {
                          setBalanceFocused(true);
                          const raw = qtyOverrides["BALANCE_OVERRIDE"] !== undefined ? String(qtyOverrides["BALANCE_OVERRIDE"]) : String(balance);
                          e.target.value = raw;
                          setTimeout(() => e.target.select(), 0);
                        }}
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center">
          ...
          >
            ...
            <div className="mt-4 text-left">
              <button
                onClick={() => {
                  // Count existing payment rows to determine the next number
                  const paymentCount = events.filter(e => /^\d+(st|nd|rd|th) payment$/i.test(e.title)).length;
                  const nextNum = paymentCount + 1;
                  const suffix = nextNum === 1 ? "st" : nextNum === 2 ? "nd" : nextNum === 3 ? "rd" : "th";
                  setEvents([...events, { title: `${nextNum}${suffix} payment`, value: "" }]);
                }}
